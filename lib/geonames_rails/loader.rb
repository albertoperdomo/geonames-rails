module GeonamesRails
  ...
  class Loader
    ...
    def initialize(writer, logger = nil)
  ...
  protected
    ...
    def load_alternate_names
      ...
      alternate_names = []
      count = 0
      File.open(File.join(RAILS_ROOT, 'tmp', 'alternateNames.txt'), 'r') do |f|
        f.each_line do |line|
          parts = line.split("\t")
          next unless parts[2].present? # no language specified
          next unless ["es", "en"].include?parts[2] #only spanish and english for now
